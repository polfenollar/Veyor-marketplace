...
import com.veyor.marketplace.modules.identity.OrganizationRepository;
import com.veyor.marketplace.modules.booking.Booking;
import com.veyor.marketplace.modules.booking.BookingRepository;
import com.veyor.marketplace.modules.booking.Shipment;
import com.veyor.marketplace.modules.catalog.Tariff;
import com.veyor.marketplace.modules.catalog.TariffRepository;
import com.veyor.marketplace.modules.catalog.CommodityTemplateRepository;
import com.veyor.marketplace.modules.booking.ShipmentRepository;
import com.veyor.marketplace.modules.shipping.TrackingEventRepository;
import com.veyor.marketplace.modules.shipping.ShipmentDocumentRepository;
import com.veyor.marketplace.modules.shipping.ShipmentTaskRepository;
import com.veyor.marketplace.modules.content.NotificationRepository;
import com.veyor.marketplace.modules.crm.CompanyProfileRepository;
import com.veyor.marketplace.modules.crm.ReferralRepository;
import com.veyor.marketplace.modules.finance.CreditRepository;
...
public class DataInitializer implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        // 1. SUPER_ADMIN
        if (userRepository.findByEmail("admin@veyor.com").isEmpty()) {
            User admin = new User("admin@veyor.com", "admin123", "SUPER_ADMIN", null);
            userRepository.save(admin);
            System.out.println("Seeded SUPER_ADMIN User: admin@veyor.com / admin123");
        } else {
            // Migrate existing ADMIN to SUPER_ADMIN
            User admin = userRepository.findByEmail("admin@veyor.com").get();
            if ("ADMIN".equals(admin.getRole())) {
                admin.setRole("SUPER_ADMIN");
                userRepository.save(admin);
                System.out.println("Migrated admin@veyor.com from ADMIN to SUPER_ADMIN");
            }
        }

        // 2. FINANCE_MANAGER
        if (userRepository.findByEmail("finance@veyor.com").isEmpty()) {
            User financeManager = new User("finance@veyor.com", "finance123", "FINANCE_MANAGER", null);
            userRepository.save(financeManager);
            System.out.println("Seeded FINANCE_MANAGER User: finance@veyor.com / finance123");
        }

        // 3. SUPPORT_AGENT
        if (userRepository.findByEmail("support@veyor.com").isEmpty()) {
            User supportAgent = new User("support@veyor.com", "support123", "SUPPORT_AGENT", null);
            userRepository.save(supportAgent);
            System.out.println("Seeded SUPPORT_AGENT User: support@veyor.com / support123");
        }
