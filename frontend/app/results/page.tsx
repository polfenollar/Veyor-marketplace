...
export default function ResultsPage() {
    ...
    useEffect(() => {
        const fetchTariffs = async () => {
            try {
                ...
                const endpoint = (origin || destination)
                    ? `/api/tariffs/search?${params.toString()}`
                    : '/api/tariffs';
                ...
                // Map backend DTO to Frontend Quote
                const mappedQuotes: Quote[] = data.map((t: any) => ({
                    id: t.id.toString(),
                    providerName: t.carrierName,
                    rating: 4.5, // Mock
                    reviewCount: Math.floor(Math.random() * 100), // Mock
                    price: t.price,
                    currency: t.currency,
                    transitTimeDays: t.transitTimeDays,
                    minDays: t.transitTimeDays,
                    maxDays: t.transitTimeDays + 2,
                    mode: t.mode.toLowerCase(),
                    tags: [],
                    route: {
                        originCode: t.originPort,
                        destinationCode: t.destinationPort,
                        stops: []
                    }
                }));

                // Combine with MOCK_QUOTES if needed, or just use mappedQuotes
                // For now, let's use mappedQuotes if available, else fallback to MOCK_QUOTES
    ...
    const onSelectQuote = (quote: Quote) => {
        alert(`Selected quote: ${quote.providerName} - ${quote.price} ${quote.currency}`);
    };
                            ...
                            <div key={quote.id} className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 transition-all hover:shadow-md hover:border-blue-300 relative">
                                ...
                                <div className="flex flex-col md:flex-row justify-between items-start mt-4">
                                    ...
                                    <div className="mt-6 md:mt-0 flex flex-col items-end">
                                        ...
                                        <button
                                            onClick={() => router.push(`/booking/${quote.id}`)}
